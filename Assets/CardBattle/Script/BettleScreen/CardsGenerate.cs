...
{ 
    ...
    void Awake()
    {
        ...
        ShuffleCards();
        bettleManager= GameObject.Find("ValueOfGame").GetComponent<BettleManager>();;
    ...
    void ShipsIns()
    {
        ...
        for (int i=0;i<4;i++)
        {
            ...
            InstallIns(shipItems[shipIndex_Player[i]], shipIns_Player[i],1);
            InstallIns(shipItems[shipIndex_Player[i]], shipIns_Enemy[i],1);
        }

    }
    //
    void InstallIns(ShipItem shipItem,Transform shipIns,int level)
    {
        Ship ship= shipIns.transform.GetComponent<Ship>();
        ship.HP = shipItem.HP*level;
        ship.AP = shipItem.Alpha*level;
        ship.ATK = shipItem.ATK*level;
